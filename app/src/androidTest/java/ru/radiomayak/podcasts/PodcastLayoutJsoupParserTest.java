...
public class PodcastLayoutJsoupParserTest {
    private static final String RESPONSE_URL = "http://example.com";

    private PodcastLayoutJsoupParser parser;

    @Before
    public void before() {
        parser = new PodcastLayoutJsoupParser();
    }
    ...
    public void shouldGetRecords() throws IOException {
        ...
        try (InputStream stream = getResource("podcasts/podcast-1.html")) {
            PodcastLayoutContent content = parser.parse(IOUtils.toByteArray(stream), "UTF-8", RESPONSE_URL);
