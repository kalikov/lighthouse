...
import ru.radiomayak.http.MessageConstraintException;
 ...
 * @since 4.0
 */
public abstract class AbstractMessageParser<T extends HttpMessage> implements ru.radiomayak.http.io.HttpMessageParser<T> {

    private static final int HEAD_LINE    = 0;
    private static final int HEADERS      = 1;

    private final SessionInputBuffer sessionBuffer;
    private final ru.radiomayak.http.config.MessageConstraints messageConstraints;
    private final List<ru.radiomayak.http.util.CharArrayBuffer> headerLines;
    protected final ru.radiomayak.http.message.LineParser lineParser;
    ...
    public AbstractMessageParser(
        ...
        this.messageConstraints = constraints != null ? constraints : ru.radiomayak.http.config.MessageConstraints.DEFAULT;
        this.headerLines = new ArrayList<ru.radiomayak.http.util.CharArrayBuffer>();
    ...
    public static Header[] parseHeaders(
            ...
            final ru.radiomayak.http.message.LineParser parser) throws HttpException, IOException {
        final List<ru.radiomayak.http.util.CharArrayBuffer> headerLines = new ArrayList<ru.radiomayak.http.util.CharArrayBuffer>();
    ...
    public static Header[] parseHeaders(
            ...
            final int maxHeaderCount,
            final int maxLineLen,
            final ru.radiomayak.http.message.LineParser parser,
            final List<ru.radiomayak.http.util.CharArrayBuffer> headerLines) throws HttpException, IOException {
        Args.notNull(inbuffer, "Session input buffer");
        Args.notNull(parser, "Line parser");
        Args.notNull(headerLines, "Header line list");

        ru.radiomayak.http.util.CharArrayBuffer current = null;
        ru.radiomayak.http.util.CharArrayBuffer previous = null;
        for (;;) {
            if (current == null) {
                current = new ru.radiomayak.http.util.CharArrayBuffer(64);
            } else {
                current.clear();
            }
        ...
        for (int i = 0; i < headerLines.size(); i++) {
            final ru.radiomayak.http.util.CharArrayBuffer buffer = headerLines.get(i);
            try {
