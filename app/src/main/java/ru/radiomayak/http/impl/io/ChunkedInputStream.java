...
import ru.radiomayak.http.TruncatedChunkException;
import ru.radiomayak.http.Header;
import ru.radiomayak.http.HttpException;
import ru.radiomayak.http.MalformedChunkCodingException;
...
public class ChunkedInputStream extends InputStream {
    ...
    private final ru.radiomayak.http.io.SessionInputBuffer in;
    private final ru.radiomayak.http.util.CharArrayBuffer buffer;
     ...
     * @since 4.4
     */
    public ChunkedInputStream(final ru.radiomayak.http.io.SessionInputBuffer in, final ru.radiomayak.http.config.MessageConstraints constraints) {
        super();
        this.in = ru.radiomayak.http.util.Args.notNull(in, "Session input buffer");
        this.pos = 0L;
        this.buffer = new ru.radiomayak.http.util.CharArrayBuffer(16);
     ...
     * @param in The session input buffer
     */
    public ChunkedInputStream(final ru.radiomayak.http.io.SessionInputBuffer in) {
        this(in, null);
    }

    @Override
    public int available() throws IOException {
        if (this.in instanceof ru.radiomayak.http.io.BufferInfo) {
            final int len = ((ru.radiomayak.http.io.BufferInfo) this.in).length();
    ...
    private long getChunkSize() throws IOException {
        ...
        case CHUNK_LEN:
            ...
            if (bytesRead2 == -1) {
                throw new ru.radiomayak.http.ConnectionClosedException("Premature end of chunk coded message body: " +
