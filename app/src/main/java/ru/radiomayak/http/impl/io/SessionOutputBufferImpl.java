 ...
 * <p>
 * {@link #writeLine(ru.radiomayak.http.util.CharArrayBuffer)} and {@link #writeLine(String)} methods
 * of this class use CR-LF as a line delimiter.
 *
 * @since 4.3
 */
public class SessionOutputBufferImpl implements ru.radiomayak.http.io.SessionOutputBuffer, ru.radiomayak.http.io.BufferInfo {
    ...
    public SessionOutputBufferImpl(
        ...
        super();
        ru.radiomayak.http.util.Args.positive(buffersize, "Buffer size");
        ru.radiomayak.http.util.Args.notNull(metrics, "HTTP transport metrcis");
    ...
    private void streamWrite(final byte[] b, final int off, final int len) throws IOException {
        ru.radiomayak.http.util.Asserts.notNull(outstream, "Output stream");
     ...
     */
    @Override
    public void writeLine(final ru.radiomayak.http.util.CharArrayBuffer charbuffer) throws IOException {
        if (charbuffer == null) {
            return;
        }
        if (this.encoder == null) {
