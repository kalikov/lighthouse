...
import ru.radiomayak.http.MethodNotSupportedException;
import ru.radiomayak.http.annotation.Contract;
import ru.radiomayak.http.util.EncodingUtils;
import ru.radiomayak.http.util.EntityUtils;
...
import ru.radiomayak.http.UnsupportedHttpVersionException;
...
public class HttpService {
    ...
    private final ru.radiomayak.http.ConnectionReuseStrategy connStrategy;
     ...
     *   {@link ru.radiomayak.http.impl.DefaultConnectionReuseStrategy#INSTANCE} will be used.
     * @param responseFactory  the response factory. If {@code null}
     *   {@link ru.radiomayak.http.impl.DefaultHttpResponseFactory#INSTANCE} will be used.
    ...
    public HttpService(
            ...
            final ru.radiomayak.http.ConnectionReuseStrategy connStrategy,
            final HttpResponseFactory responseFactory,
            final HttpRequestHandlerMapper handlerMapper,
            final HttpExpectationVerifier expectationVerifier) {
        super();
        this.processor =  ru.radiomayak.http.util.Args.notNull(processor, "HTTP processor");
        this.connStrategy = connStrategy != null ? connStrategy :
            ru.radiomayak.http.impl.DefaultConnectionReuseStrategy.INSTANCE;
        ...
        try {
            ...
            if (request instanceof HttpEntityEnclosingRequest) {
                ...
                if (((HttpEntityEnclosingRequest) request).expectContinue()) {
                    response = this.responseFactory.newHttpResponse(HttpVersion.HTTP_1_1, HttpStatus.CONTINUE.getCode(), context);
                    if (this.expectationVerifier != null) {
                        try {
                            this.expectationVerifier.verify(request, response, context);
                        } catch (final HttpException ex) {
                            response = responseFactory.newHttpResponse(HttpVersion.HTTP_1_0, HttpStatus.INTERNAL_SERVER_ERROR.getCode(), context);
            ...
            if (response == null) {
                response = this.responseFactory.newHttpResponse(HttpVersion.HTTP_1_1, HttpStatus.OK.getCode(), context);
            ...
            response = this.responseFactory.newHttpResponse(HttpVersion.HTTP_1_0, HttpStatus.INTERNAL_SERVER_ERROR.getCode(), context);
            handleException(ex, response);
        }
    ...
    protected void handleException(final HttpException ex, final HttpResponse response) {
        ...
        final ru.radiomayak.http.entity.ByteArrayEntity entity = new ru.radiomayak.http.entity.ByteArrayEntity(msg);
