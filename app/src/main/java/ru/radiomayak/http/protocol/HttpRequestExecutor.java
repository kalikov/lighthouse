...
public class HttpRequestExecutor {
    ...
    public HttpRequestExecutor(final int waitForContinue) {
        super();
        this.waitForContinue = ru.radiomayak.http.util.Args.positive(waitForContinue, "Wait for continue time");
    }
    ...
    public HttpResponse execute(
        ...
        ru.radiomayak.http.util.Args.notNull(conn, "Client connection");
        ru.radiomayak.http.util.Args.notNull(context, "HTTP context");
        try {
    ...
    public void preProcess(
        ...
        ru.radiomayak.http.util.Args.notNull(request, "HTTP request");
        ru.radiomayak.http.util.Args.notNull(processor, "HTTP processor");
        ru.radiomayak.http.util.Args.notNull(context, "HTTP context");
            ...
            if (((HttpEntityEnclosingRequest) request).expectContinue() && !ver.lessEquals(HttpVersion.HTTP_1_0)) {
                ...
                if (conn.isResponseAvailable(this.waitForContinue)) {
                    ...
                    if (status < HttpStatus.OK.getCode()) {
                        if (status != HttpStatus.CONTINUE.getCode()) {
    ...
    protected HttpResponse doReceiveResponse(
        ...
        while (response == null || statusCode < HttpStatus.OK.getCode()) {
