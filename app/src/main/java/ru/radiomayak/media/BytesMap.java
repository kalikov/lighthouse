...
class BytesMap {
    ...
    BytesMap(int capacity) {
        this.capacity = capacity;
        segments = EMPTY_SEGMENTS;
    }

    BytesMap(int[] segments) {
        this(0, segments);
    }

    BytesMap(int capacity, int[] segments) {
    ...
    BytesMap(int from, int to) {
        this(0, from, to);
    }

    BytesMap(int capacity, int from, int to) {
    ...
    public ByteBuffer getBytes(HttpRange range) {
        return getBytes(range.getFrom(), range.getTo());
    }

    public ByteBuffer getBytes(int from, int to) {
        int offset = 0;
        for (int i = 0; i < segments.length; i += 2) {
            int start = segments[i];
            int end = i + 1 < segments.length ? segments[i + 1] : start + bytes.length - offset - 1;
            if (from >= start && from <= end && to <= end) {
                return ByteBuffer.wrap(bytes, offset + from - start, to - from + 1);
            }
            if (from < start) {
                break;
            }
